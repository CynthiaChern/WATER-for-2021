...
def image_callback(message):
    ...
    # print(frame.shape[1])
    if (max_c_area>40):
        velocity_message.linear.x= linear_speed_factor/max_c_area *8
        Az = (x-frame.shape[1]/2)* angular_speed_factor ;
        integrated_angular_speed +=Az
        if abs(Az)>0.1:
            velocity_message.angular.z= (Az + integrated_angular_factor * integrated_angular_speed)/5
        else:
...
def camera_listener():
    ...
    rospy.Subscriber("/water_astra_rgbd/rgb/image_raw", Image, image_callback)
    rospy.spin()
