...
class Follower:
	def __init__(self):
		...
		self.controllerLossTimer = threading.Timer(30, self.controllerLoss) #if we lose connection
		...
		#PID_param = yaml.load('simple_follower/parameters/PID_param.yaml')
		...
		self.PID_controller = simplePID([0, targetDist], [0.7, 0.6], [0.07, 0.04], [0.0,0.0])
